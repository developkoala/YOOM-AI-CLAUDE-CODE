...
export async function performUpdate(options?: {
  ...
  try {
    ...
    const tempScript = join(tempDir, `yoom-ai-update-${Date.now()}.sh`);
...
const SILENT_UPDATE_STATE_FILE = join(CLAUDE_CONFIG_DIR, '.yoom-ai-silent-update.json');
...
export async function silentAutoUpdate(config: SilentUpdateConfig = {}): Promise<UpdateResult | null> {
  const {
    ...
    autoApply = true,
    logFile = join(CLAUDE_CONFIG_DIR, '.yoom-ai-update.log'),
    maxRetries = 3
  } = config;
